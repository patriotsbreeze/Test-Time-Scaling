...
\begin{description}
  \item[H1 (Replication)] $\tax \neq 0$ for coding agents; specifically, we test whether
    $\tax > 0$ as reported by ASPI for frontier general agents.
  \item[H2 (Scale Dependence)] The sign and magnitude of $\tax$ depend on model scale
    and instruction-hierarchy training, with sub-frontier models potentially showing
    $\tax < 0$.
  \item[H3 (Mitigation)] Provenance-aware defenses reduce clarification-channel ASR
    significantly without eliminating collaboration utility.
