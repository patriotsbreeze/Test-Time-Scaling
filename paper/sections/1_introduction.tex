...
\begin{enumerate}
  ...
  \item \textbf{Empirical validation.} Experiments with 50 independent runs confirm
    ...
    ($T \gg K^D$). On $100$ real GSM8K problems (Math-Shepherd PRM), DVA-MCTS
    achieves $8.8\%$ fewer calls while improving accuracy by $3.6$ percentage points,
    with larger call savings expected for PRMs with continuous score outputs
...
reduction. On real Math-Shepherd PRM data, the call reduction is $8.8\%$ (with an additional
$3.6$-percentage-point accuracy improvement) due to the proxy-smoothing effect;
the modest call savings reflect the high effective Lipschitz constant of binary step labels; PRMs with smooth, continuous
score outputs are expected to yield larger savings, as their score trajectories
satisfy the Lipschitz assumption more tightly. These efficiency gains are
particularly valuable when the verifier is a large neural network (e.g., a $7$B
