...
\begin{enumerate}
  ...
  \item \textbf{Regret bound.} Under a Lipschitz score continuity assumption on
    ...
    $N_{\mathcal{T}} = K^D - 1$ times total, so the call fraction $\mathcal{B}/T \to 0$
  ...
  \item \textbf{Empirical validation.} Experiments with 50 independent runs confirm
    ...
    In the large-budget synthetic regime ($B{=}3200$, $K^D{=}4{,}096$ nodes),
    DVA-MCTS reduces verifier calls by $73\%$ with a $2$-percentage-point accuracy
    gap; savings grow with budget as the algorithm enters the exploitation regime
    ($T \gg K^D$). An adaptive online estimator for $L$ closes $97\%$ of the regret
    gap caused by $L$ misspecification, recovering full accuracy with only $1\%$
    additional verifier calls. On $100$ real GSM8K problems (Math-Shepherd PRM),
    DVA-MCTS achieves $8.8\%$ fewer calls \emph{and} $3.6$ percentage points higher
    accuracy than exhaustive verification, with larger call savings expected for PRMs
    with continuous score outputs
...
gap caused by $L$ misspecification. On real Math-Shepherd PRM data, DVA-MCTS
simultaneously reduces verifier calls by $8.8\%$ \emph{and} improves accuracy by
$3.6$ percentage points relative to exhaustive verification---a Pareto improvement
in both efficiency and quality. The modest call savings on binary-label PRMs are
expected to grow substantially for PRMs with continuous score outputs, where the
effective $L$ is lower and the threshold skips more verification steps. These gains
are particularly valuable when the verifier is a large neural network (e.g., a $7$B
PRM) executed on dedicated hardware.
