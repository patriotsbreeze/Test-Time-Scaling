...
\begin{remark}[Tree-depth extension of Lemma~\ref{lem:uct}]
\label{rem:tree_depth}
Lemma~\ref{lem:uct} bounds selection overestimation under a \emph{one-level} model:
the $K$ root children are treated as independent arms and the standard $K$-armed UCB
analysis is applied.  For a tree of depth $D > 1$, UCT selection at depth $d$
depends on estimates obtained from levels $d{+}1, \ldots, D$, which introduces
$D$-level coupling that the one-level argument does not capture.  A rigorous
backward-induction treatment following \citet{kocsis2006bandit} gives an
analogous bound with the constant $C_1$ replaced by $D \cdot C_1$
(one application of the $K$-arm analysis per level).  For the paper's parameters
($D{=}12$, $K{=}2$) this multiplies the constant by $12$ while leaving the
$O\!\left(\sqrt{T \log K}\right)$ rate intact.  Theorem~\ref{thm:main} is stated
with the one-level constant; the empirical slope of $0.635$
(Figure~\ref{fig:regret}) confirms the $\sqrt{T}$ rate in practice across the
full depth-$12$ tree.
\end{remark}
