...
\[
  \mathbb{E}[R(T)] \;\leq\; C_1(K,D) \sqrt{T \log K}
  \;+\; 2\gamma \sqrt{T},
\]
where $C_1(K,D) = 8\sigma_{\max}\sqrt{2}\,\Phi(K,D)$,
$\Phi(K,D) = (K^{D/2}-1)/(\sqrt{K}-1)$ is a tree-geometry constant
(with $\Phi(K,1) = 1$), and $K$, $D$ are the branching factor and depth.
Setting $\gamma^* = C_1(K,D)\sqrt{\log K}/2$ equates the two terms:
\[
  \mathbb{E}[R(T)] \;\leq\; C(K,D)\sqrt{T \log K},
  \qquad C(K,D) = 16\sigma_{\max}\sqrt{2}\,\Phi(K,D).
\]
For fixed $K$ and $D$, $C(K,D)$ is a constant independent of $T$, confirming the
$O\!\left(\sqrt{T\log K}\right)$ rate.
\end{theorem}
...
\]
Setting $\gamma^* = C_1(K,D)\sqrt{\log K}/2$ gives
$2\gamma^*\sqrt{T} = C_1(K,D)\sqrt{T\log K}$, yielding
$\mathbb{E}[R(T)] \leq C(K,D)\sqrt{T\log K}$ with $C(K,D) = 2C_1(K,D) = 16\sigma_{\max}\sqrt{2}\,\Phi(K,D)$.
...
the tree-geometry factor $\Phi(K,D)$, which is a constant for fixed $K, D$.
\end{corollary}
