...
\label{cor:optimal}
DVA-MCTS achieves the minimax-optimal regret \emph{rate} $\Theta(\sqrt{T \log K})$:
the upper bound (Theorem~\ref{thm:main}) and lower bound (Theorem~\ref{thm:lower})
agree on the rate up to $\sqrt{\log K}$.  The constant $C(K,D)$ is conservative
by a factor $\Phi(K,D)$ relative to the tight $D{=}1$ value; this is a proof artifact
(Remark~\ref{rem:constant_tightness}), not a property of the algorithm.
\end{corollary}
...
tree size $N_{\mathcal{T}} = K(K^D-1)/(K-1)$:
...
$N_{\mathcal{T}}{=}8{,}190$), budgets $B \leq 400$ are firmly in the exploration
